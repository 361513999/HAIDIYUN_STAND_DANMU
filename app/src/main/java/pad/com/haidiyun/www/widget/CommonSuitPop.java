...
public class CommonSuitPop {
	...
	public CommonSuitPop(Context context, FouceBean foodsBean, TcT tcT) {
	...
	public void showSheet() {
		...
		LinearLayout layout = (LinearLayout) inflater.inflate(
				R.layout.flip_suit_pop_view, null);
			...
			public void onClick(View v) {
				...
				for (int i = 0; i < selbean.size(); i++) {
					if (i == selbean.size() - 1) {
						...
						detailNames +=selbean.get(i).getName();
					} else {
						details += selbean.get(i).getCode() + ",";
						detailNames +=selbean.get(i).getName()+",";
					}
				}
				tcT.insert(foodsBean, details,detailNames);
				close();
			}
		});
