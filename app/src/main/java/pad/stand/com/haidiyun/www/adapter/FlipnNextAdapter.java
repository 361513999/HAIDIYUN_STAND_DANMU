...
import pad.stand.com.haidiyun.www.widget.MapAreaView;
...
//	  viewHolder.img.setImageBitmap(BitmapFactory.decodeFile(flipBeans.get(position).getPath()));
	  Glide.with(BaseApplication.application).load("file://"+flipBean.getPath()).asBitmap().diskCacheStrategy(DiskCacheStrategy.RESULT).into(viewHolder.img);
	  return convertView;
