...
import pad.stand.com.haidiyun.www.adapter.SuitItemAdapter;
...
public class CommonSelectSuitPop {
    ...
    public CommonSelectSuitPop(Context context, FoodsBean foodsBean, TcT t, String num) {
...
//					tcAdapter.init();
    ...
    private void selectItem(final int inde) {
        new Thread() {
            ...
            public void run() {
                super.run();
                tcBeans.clear();
                tcBeans = DB.getInstance().getTc(itemBeans.get(inde).getCode(), foodsBean.getChargeMode());
                tcBeansList.set(inde, DB.getInstance().getTc(itemBeans.get(inde).getCode(), foodsBean.getChargeMode()));
    ...
    public void showSheet() {
        ...
        LinearLayout layout = (LinearLayout) inflater.inflate(
                R.layout.select_suit_pop_view, null);
        ...
        save.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                for (int i = 0; i < maps.size(); i++) {
                    for (int j = 0; j < maps.get(i).size(); j++) {
                        if (maps.get(i).get(j)) {
                            ...
                            totalPrice = totalPrice + tcBeansList.get(i).get(j).getPrice();
                ...
                t.insert(foodsBean, builder.toString(), builder1.toString(), num);
                close();
