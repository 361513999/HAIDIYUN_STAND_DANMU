...
package pad.stand.com.haidiyun.www.curl;
...
public class CurlView extends GLSurfaceView implements View.OnTouchListener,
	...
	public boolean onTouch(View view, MotionEvent me) {
		...
		case MotionEvent.ACTION_UP: {
			...
			System.out.println("UP    "+me.getX()+"----"+me.getY());
			if (mCurlState == CURL_LEFT || mCurlState == CURL_RIGHT) {
				...
				System.out.println("mPointerPos.mPos.x"+mPointerPos.mPos.x);
				System.out.println("leftRect.left"+leftRect.left+"--"+leftRect.right);
				System.out.println("rightRect.left"+rightRect.left+"--"+rightRect.right);
				int CENTER = screenWidth/2;
				//改造了滑动策略
				if ((mViewMode == SHOW_ONE_PAGE && mPointerPos.mPos.x>0&&mPointerPos.mPos.x >0.8 && CENTER<DOWN_FIG )||(mViewMode == SHOW_ONE_PAGE &&  mPointerPos.mPos.x >-0.8&& CENTER>DOWN_FIG )
						|| (mViewMode == SHOW_TWO_PAGES
						&& mPointerPos.mPos.x > rightRect.left)) {
					// On right side target is always right page's right border.
					System.out.println("点了这个");
					mAnimationTarget.set(mDragStartPos);
					mAnimationTarget.x = mRenderer
							.getPageRect(CurlRenderer.PAGE_RIGHT).right;
					mAnimationTargetEvent = SET_CURL_TO_RIGHT;
				} else {
					System.out.println("点了这个00");
					// On left side target depends on visible pages.
					mAnimationTarget.set(mDragStartPos);
					if (mCurlState == CURL_RIGHT || mViewMode == SHOW_TWO_PAGES) {
						System.out.println("00000");
						
						System.out.println(mAnimationTarget.x +"----"+ leftRect.left);
						
						mAnimationTarget.x = leftRect.left;
					} else {
						System.out.println("111111");
						
						System.out.println(mAnimationTarget.x +"----"+ rightRect.left);
	...
	private void updatePage(CurlPage page, int index) {
		...
		mPageProvider.updatePage(page, mPageBitmapWidth, mPageBitmapHeight,
				index%REAL_TOTAL_PAGES);
	}
	private int REAL_TOTAL_PAGES = 0;
	/**
	 * 达到无限翻页模式
	 * @param REAL_TOTAL_PAGES
	 */
	public void setTotalPages(int REAL_TOTAL_PAGES){
		this.REAL_TOTAL_PAGES = REAL_TOTAL_PAGES;
	}
	/**
